...
export const projects: Project[] = [
  ...
  },
  {
    ...
    summary:
      "A daily asynchronous crew game: make one four-fragment vibe card, judge yesterday's cards blind, then reveal authors and votes.",
    description:
      "VibeGrid is a daily asynchronous social game for private crews. Everyone receives the same four-column palette, chooses exactly four fragments, and names the combination. The next day, people who made a card judge the round without seeing authors; the day after, authors and votes are revealed. There is no answer key. The durable path uses Go and Postgres to own stage timing, membership, one-card and one-ballot rules, self-vote prevention, stable identity, replay safety, privacy, and staged disclosure. A public browser-only practice round teaches the loop without pretending to be a durable crew.",
    metrics: [
      { label: "Loop", value: "Make → judge → reveal" },
      { label: "Crew size", value: "1–20 members" },
      { label: "Retries", value: "Idempotent writes" },
      { label: "Runtime", value: "Go + Postgres" },
    ],
    facts: [
      { label: "Scope", value: "Private daily crew ritual with public browser-only practice" },
      { label: "Role", value: "Product mechanic, staged interaction, transactional rules, identity, privacy, and operations" },
      { label: "Status", value: "Public repo; deploy-ready locally, permanent production URL not claimed" },
      { label: "Proof", value: "Product vision, decision register, service tests, migrations, smoke script, and runbooks" },
      { label: "Best fit", value: "Backend/product roles with social state, privacy, and correctness-heavy writes" },
    ],
    system: [
      { label: "Board state", value: "An immutable dated palette whose row count freezes from the crew-size snapshot" },
      { label: "Make", value: "Exactly four fragments and one title, with one card allowed per member and board" },
      { label: "Judge", value: "Eligible makers see authors hidden and cast one non-self ballot" },
      { label: "Reveal", value: "Authors and votes appear only in the result stage; ties remain ties" },
      { label: "Identity", value: "Revocable hash-only browser bearers map to stable crew identities across leave and rejoin" },
      { label: "Runtime", value: "A Go binary embeds the exported Next.js frontend and migrations; Postgres owns the durable crew path" },
    ],
    evidenceNote:
      "The strongest proof is the product contract: staged disclosure, transaction-owned authorship and voting, replay-safe retries, stable identity, and an honest public-practice boundary.",
    evaluationPath: [
      "Open the GitHub repo and play the browser-only practice round described in the README.",
      "Read the product vision and decision register for the make, judge, reveal contract and deliberate non-goals.",
      "Inspect the round handlers, transactional store, stable-identity migration, and stage-specific privacy projections.",
      "Run the documented service, security, Postgres, build, and smoke checks before making deployment claims.",
    ],
    highlights: [
      "Reframed the product from a solitary answer-key puzzle into a private, asynchronous social ritual where the crew's interpretations are the point.",
      "Made Go and Postgres authoritative for membership, stage timing, one-card and one-ballot limits, no-self-vote rules, ties, and result thresholds.",
      "Used client replay IDs, database uniqueness, and stable member identities so timed-out retries or membership churn cannot reset product rules.",
      "Separated public practice from durable crews and added hash-only revocable sessions, CSRF protection, rate limits, health/readiness probes, metrics, and runbooks.",
    ],
    sections: [
      {
        title: "Product perspective",
        paragraphs: [
          "VibeGrid gives a crew one creative constraint per day. Each person makes a named four-fragment card, judges the previous day's cards without author labels, and returns for the reveal. The suspense comes from interpretation and recognition, not from recovering a concealed answer.",
          "The public homepage is a complete practice round, not a signup gate. Durable crews remain private, and the interface reveals only what the current stage and membership allow.",
        ],
      },
      {
        title: "Technical perspective",
        paragraphs: [
          "The Go service owns the social contract: immutable dated boards, authorization, submission and ballot transactions, staged projections, privacy, identity, and replay semantics. Postgres constraints reinforce same-round targets, one-card and one-vote rules, and no-self-vote behavior.",
          "The exported Next.js frontend and migrations ship inside one non-root Go container. Operations include structured logs, health and readiness probes, bounded metrics, graceful shutdown, retention cleanup, and explicit production guardrails.",
        ],
      },
      {
        title: "How I would present it",
        paragraphs: [
          "I would start with the three-day interaction and show how one card changes across make, blind judge, and reveal. Then I would trace the same rule through UI projection, Go authorization, the transaction, and the database constraint.",
          "The strongest engineering discussion is about state that must survive retries and membership churn without leaking authorship early or inventing a result for a quiet round.",
        ],
      },
    ],
